...
app.post("/transaction", (req, res) => {
  ...
  console.log(`Request transaction process with id = ${req.body.id}`);
  ...
  axios
    ...
    .catch((e) => {
      if (e.response.status === 504) {
        ...
        axios
          .put(`${clientUrl}/transaction`, {
            status: { id, status: "timeout" },
          })
          .catch(() => {
            console.log("Error with calling client.");
          });
...
app.post("/hookTxStatus", (req, res) => {
  console.log("Hook for long latency response");
  res.send();
});
